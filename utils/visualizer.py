...
class Visualizer():
    ...
    def display_current_pmt(self, epoch, visuals,is_resize=False):
        
        images,prompted_img, prompt, entropy_maps, preds,gt_segs,refined_pseu = visuals['images'],visuals['prompted_img'], visuals['prompt'],visuals['entropy_maps'],visuals['preds'],visuals['gt_segs'], visuals['refined_pseu']
        concat_results = []
        prompt = np.repeat(prompt, 3, 1)
        for i in range(min(images.shape[0],8)):
            img = images[i]
            if img.ndim == 2:
                img = np.stack([img,img,img],axis=-1)
            ...
            prompted_img_i = prompted_img[i].transpose((1, 2, 0))
            prompted_img_i = (prompted_img_i - prompted_img_i.min()) / (prompted_img_i.max() - prompted_img_i.min())
            prompted_img_i = (prompted_img_i*255).astype(np.uint8)

            diff = prompted_img_i - img
            diff = (diff - diff.min()) / (diff.max()-diff.min())
            diff = (diff*255).astype(np.uint8)

            prompt_i = prompt[0].transpose((1, 2, 0))
            prompt_i = (prompt_i - prompt_i.min()) / (prompt_i.max() - prompt_i.min())
            prompt_i = (prompt_i*255).astype(np.uint8)
            ...
            refined_pseu_i = self.add_mask(img, refined_pseu[i].astype(np.uint8))
            concat_results.append(np.concatenate([img,prompted_img_i,diff, prompt_i,img_uncertain,img_pred,img_gt, refined_pseu_i],axis=1))
        concat_results = np.concatenate(concat_results,axis=0)
        if is_resize:
            concat_results = cv2.resize(concat_results,dsize=None,fx=0.5,fy=0.5,interpolation=cv2.INTER_LINEAR)
        cv2.imwrite(os.path.join(self.opt['checkpoint_dir'], 'visuals',  'img_ent_pred_gt' + str(epoch) + '.png'),concat_results[:,:,::-1])
