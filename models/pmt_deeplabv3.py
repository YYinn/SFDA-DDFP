...
https://github.com/Deeachain/Segmentation-Pytorch/blob/master/model/DeeplabV3Plus.py
"""
import math
import torch
...
class ResNet(nn.Module):
    def __init__(self, opt, block, layers, os=16):
        self.inplanes = 64
        ...
        if not (opt['config_file'].startswith('configs/train_source_seg') or opt['config_file'].startswith('configs/test_source_seg')):
            ...
            for name, module in self.named_modules():
                if (isinstance(module, torch.nn.BatchNorm2d) or isinstance(module, SynchronizedBatchNorm2d)) and not name.startswith('freconv'):
...
class Deeplabv3plus(nn.Module):
    def __init__(self, opt,  backbone, num_classes=3, os=16, freeze_bn=False):
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       ...
                                       BatchNorm2d(256),
                                       nn.ReLU())
        self.seg_conv = nn.Conv2d(256, num_classes, kernel_size=1, stride=1)
    ...
    def forward(self, input, type=None, phase=None):
        ...
        if type == 'None' or type == None:
            prompted_img = input
            prompt_a = torch.zeros_like(input).cuda()
        elif type == 'Spatial':
            prompted_img, prompt_a = self.spa_prompt(input)
        elif type == 'Domain' or type == 'Data':
            prompted_img, prompt_a = self.fre_prompt(input, type=type, phase=phase)
        else:
            raise ValueError(f'Please input Domain | Data | spacial as type, but got {type} instead ')

        x, low_level_features = self.resnet_features(prompted_img)
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
        ...
        return x0, x, prompted_img, prompt_a
...
def pmt_Deeplabv3plus_res50(opt, num_classes=3, os=16, pretrained=False):
    resnet50 = ResNet(opt, Bottleneck, [3, 4, 6, 3], os)
    if pretrained:
        pretrain_dict = model_zoo.load_url(model_urls['resnet50'])
        model_dict = {}
        state_dict = resnet50.state_dict()
        for k, v in pretrain_dict.items():
            if k in state_dict and k != 'conv1.weight':
                model_dict[k] = v
        state_dict.update(model_dict)
        resnet50.load_state_dict(state_dict)
    model = Deeplabv3plus(opt, backbone=resnet50, num_classes=num_classes, os=os, freeze_bn=False)
    return model
...
#     model = pmt_Deeplabv3plus_res50(num_classes=3, os=16, pretrained=False)
